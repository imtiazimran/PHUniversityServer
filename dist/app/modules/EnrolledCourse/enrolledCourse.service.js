...
const getAllEnrolledCoursesFromDB = (facultyId, query) => __awaiter(void 0, void 0, void 0, function* () {
    const faculty = yield faculty_model_1.Faculty.findOne({ id: facultyId });
    if (!faculty) {
    ...
    const enrolledCourseQuery = new QueryBuilder_1.default(enrolledCourse_model_1.default.find({
        faculty: faculty._id,
    }).populate('semesterRegistration academicSemester academicFaculty academicDepartment offeredCourse course student faculty'), query)
        .filter()
        .sort()
        .paginate()
        .fields();
